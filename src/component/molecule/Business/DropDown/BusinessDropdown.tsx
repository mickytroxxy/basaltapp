        ...
        <View style={{}}>
            ...
            <TouchableOpacity onPress={() => setIsItemListShown(!isItemListShown)} style={[{justifyContent:'center',flexDirection:'row',backgroundColor:colors.white,borderColor: isItemListShown ? colors.momoBlue : (!props.hasError ? '#525252' : colors.red80),height:66,width:'100%'},!isItemListShown && styles.isHidden, isItemListShown && styles.isNotHidden , props.hasError && {borderWidth:2}]}>
                <View style={{flex:1,justifyContent:'center',marginLeft:gpsh('16')}}>
                    ...
                    <Text  fontFamily={props.fontFamily ? props.fontFamily : 'MTNBrighterSans-Bold'} style={{color:!props.hasError ? (props.fontColor ? props.fontColor : colors.momoBlue) : '#C2334D'}} fontSize={getFontSizeByWindowWidth(14)}>{`${selectedOption ? selectedOption?.maintitle : props.placeholder} ${(!selectedOption && props.required) ? '*' : ''}`}</Text>
                </View>
                <View style={{marginRight:gpsh('16'),justifyContent:'center'}}>
                    {!isItemListShown ? <DropdownCloseIcon width={28} height={28}/> : <DropdownOpenIcon width={28} height={28}/>}
                </View>
            </TouchableOpacity>
                    ...
                    <ScrollView>
                        {listItems?.map((item,i) => 
                            <TouchableOpacity onPress={()=> handleChange(item)} key={i} style={[{flex:1,justifyContent:'center',padding:gpsh('16')},i === listItems.length - 1 && styles.selectedIsLast]}>
                                ...
                                <Text fontFamily={props.fontFamily ? props.fontFamily : 'MTNBrighterSans-Bold'} color={props.fontColor ? props.fontColor : 'momoBlue'} fontSize={getFontSizeByWindowWidth(14)}>{`${selectedOption ? selectedOption?.maintitle : props.placeholder} ${(!selectedOption && props.required) ? '*' : ''}`}</Text>
                            </TouchableOpacity>
                        )}
                    </ScrollView>
                </View>
